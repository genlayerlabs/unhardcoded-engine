...
t.test("filter atoms lower to field observations", function()
    ...
    t.eq(enc(E.filter({ provider_in = { "openrouter", "openai" } })),
         enc({ "or", { "provider_eq", "openrouter" }, { "provider_eq", "openai" } }),
    ...
    t.eq(enc(E.filter({ provider_not_in = { "antseed" } })),
        ...
        "provider_not_in lowers to not(or(provider_eq)) — drop the slow marketplace")
